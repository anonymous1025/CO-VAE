...
def GraphConvolution_adj(adj,input_,output_dim, stddev=0.02,
    ...
    with tf.variable_scope(name):
        ...
        new_x=tf.tile(tf.reshape(tf.matmul(input_, w_),[FLAGS.batch_size,1,76,-1]),[1,adj.get_shape()[-1],1,1])
        conv = tf.matmul(tf.transpose(adj,[0,3,1,2]),new_x)
        
        outputs = lrelu(conv)
        #biases = tf.get_variable('biases', [output_dim], initializer=tf.constant_initializer(0.0))
        
        return tf.reshape(tf.transpose(outputs,[0,2,1,3]),[FLAGS.decoder_batch_size,76,1,FLAGS.hidden2*FLAGS.hidden2_2])
...
class n2g(Layer):
    ...
    def __init__(self,input_dim, batch_size,dropout=0., act=tf.nn.relu, **kwargs):
        ...
        with tf.variable_scope(self.name + '_vars'):
            self.vars['weights'] = weight_variable_glorot(input_dim,76, name="weights")
        self.vars['weights']=tf.tile(tf.reshape(self.vars['weights'],[1,input_dim,76]),[batch_size,1,1])      
        self.dropout = dropout
        self.act = act
...
class g2n(Layer):
    ...
    def __init__(self,input_dim, batch_size,dropout=0., act=tf.nn.relu, **kwargs):
        ...
        with tf.variable_scope(self.name + '_vars'):
            self.vars['weights'] = weight_variable_glorot(76,input_dim, name="weights")
        self.vars['weights']=tf.tile(tf.reshape(self.vars['weights'],[1,76,input_dim]),[batch_size,1,1])      
        self.dropout = dropout
        self.act = act
...
def n2g_adj(input_,output_dim,k_h, d_h=1, d_w=1, stddev=0.02,
           name="n2g"):
     with tf.variable_scope(name):
